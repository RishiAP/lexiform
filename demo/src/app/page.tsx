...
function CopyCommand({ text, isDark }: { text: string, isDark: boolean }) {
  ...
  return (
    <div className={`inline-flex items-center px-4 py-2.5 rounded-lg border font-mono text-sm ${isDark ? 'border-gray-800 bg-gray-900/50 text-gray-300' : 'border-gray-200 bg-white shadow-sm text-gray-700'} transition-all`}>
      <span className="mr-4 text-emerald-500 font-semibold">$ <span className={`${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{text}</span></span>
      <button 
        onClick={handleCopy} 
        className={`p-1.5 rounded-md transition-colors ${isDark ? 'hover:bg-gray-800 text-gray-400' : 'hover:bg-gray-100 text-gray-500'}`}
        title="Copy command"
      >
...
export default function Home() {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-950 text-gray-100' : 'bg-gray-50 text-gray-900'} p-4 sm:p-8 font-[family-name:var(--font-geist-sans)] transition-colors duration-200`}>
        ...
        <header className="text-center mb-2 mt-4 sm:mt-8">
          <div className="flex justify-center mb-3">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 120" width="200" height="75" role="img" aria-label="Lexiform Logo" className="drop-shadow-sm">
              ...
              <text x="130" y="79" style={{ fontFamily: "'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif", fontSize: "46px", fontWeight: 300, letterSpacing: "-0.5px" }} fill={isDark ? "#E0E7FF" : "#1E1B4B"}>
                lexi<tspan style={{ fontWeight: 600 }} fill={isDark ? "#818CF8" : "#4F46E5"}>form</tspan>
              </text>
            </svg>
          </div>
          <p className={`text-base sm:text-lg ${isDark ? "text-gray-400" : "text-gray-600"} mb-4 max-w-lg mx-auto`}>A highly polished, ultra-lightweight, headless-compatible drop-in React rich text editor built on Lexical.</p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-2">
            <div className={`inline-flex items-center px-5 py-2.5 rounded-full border ${isDark ? 'border-gray-800 bg-gray-900/50 text-gray-300' : 'border-gray-200 bg-white shadow-sm text-gray-600'} text-sm flex-wrap justify-center gap-x-2 gap-y-2`}>
              ...
              <a href="https://github.com/RishiAP/lexiform" target="_blank" rel="noopener noreferrer" className={`hover:underline font-medium flex items-center gap-1.5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
              ...
              <span className="mx-2 opacity-30 hidden sm:inline">|</span>
              <a href="https://www.npmjs.com/package/@rishiap/lexiform" target="_blank" rel="noopener noreferrer" className={`hover:underline font-medium flex items-center gap-1.5 ${isDark ? 'text-red-400' : 'text-red-600'}`}>
            ...
            <CopyCommand text="npm install @rishiap/lexiform" isDark={isDark} />
          </div>
        </header>
