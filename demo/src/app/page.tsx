...
function CopyCommand({ text }: { text: string }) {
  ...
  return (
    <div className="inline-flex items-center px-4 py-2.5 rounded-lg border font-mono text-sm border-border bg-card shadow-sm text-card-foreground transition-all">
      <span className="mr-4 text-emerald-500 font-semibold">$ <span className="text-card-foreground">{text}</span></span>
      <button 
        onClick={handleCopy} 
...
export default function Home() {
  console.log('Lexiform Demo Reloaded 5');
  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-4 sm:p-8 font-[family-name:var(--font-geist-sans)] transition-colors duration-200">
      <main className="max-w-4xl mx-auto flex flex-col gap-6 sm:gap-8 relative">
        <button 
          onClick={() => setTheme(resolvedTheme === 'dark' ? 'light' : 'dark')} 
          className="absolute right-0 top-0 p-3 rounded-full bg-card text-muted-foreground shadow-sm border border-border hover:bg-accent hover:text-accent-foreground transition-all"
          title="Toggle Theme"
        >
        ...
        <header className="text-center mb-2 mt-4 sm:mt-8">
          ...
          <p className="text-base sm:text-lg text-muted-foreground mb-4 max-w-lg mx-auto">A highly polished, ultra-lightweight, headless-compatible drop-in React rich text editor built on Lexical.</p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-2">
            <div className="inline-flex items-center px-5 py-2.5 rounded-full border border-border bg-card shadow-sm text-muted-foreground text-sm flex-wrap justify-center gap-x-2 gap-y-2">
              <span>Made with <span className="text-red-500">❤️</span> by <a href="https://github.com/RishiAP" target="_blank" rel="noopener noreferrer" className="font-semibold hover:underline text-foreground">RishiAP</a></span>
        ...
        <div className="bg-card border-border rounded-xl shadow-sm border overflow-hidden transition-colors duration-200">
          <LexicalEditor
            value={content}
        ...
        <div className="bg-zinc-950 border text-zinc-100 p-6 rounded-xl overflow-auto text-sm font-mono border-zinc-800">
          <h3 className="text-zinc-400 mb-2 uppercase text-xs font-semibold tracking-wider">Output (JSON)</h3>
